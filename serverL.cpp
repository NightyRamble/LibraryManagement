...
struct sockaddr_in addrL, udp_AddrM;
...
int main(){
    ...
    addrL.sin_port = htons(L_PORT);
    addrL.sin_addr.s_addr = inet_addr("127.0.0.1");
    if(bind(udpSkt, (struct sockaddr*)&addrL, sizeof(addrL)) == -1){
        perror("Binding UDP error");
        exit(-1);
    }
    cout << "Server L is up and running using UDP on port "<< L_PORT << endl;
    ...
    ifstream literature;
    literature.open("./literature.txt");

    if (!literature.is_open()) {
        cerr << "Failed to open the input file." << endl;
        return 1;
    }

    while (getline(literature, line)) {
        vector<string> row;
...
//        cout << bookCode << endl;
        ...
        if(admin == "Y"){
            cout << "Server L finished sending the inventory status to the Main server using UDP on port " << L_PORT << endl;
        }
        else{
            cout << "Server L finished sending the availability status of code " << bookCode << " to the Main Server using UDP on port " << L_PORT << "." << endl;
        }
    }
